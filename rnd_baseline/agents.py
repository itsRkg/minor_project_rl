...
class RNDAgent(object):
    ...
    def train_model(self, s_batch, target_ext_batch, target_int_batch, y_batch, adv_batch, next_obs_batch, old_policy):
        """Run PPO + RND update for self.epoch epochs.

        Returns a dict of training metrics (averaged over all mini-batches)
        for logging to metrics.jsonl / TensorBoard.
        """
        ...
        # accumulators for metric averaging
        acc = dict(policy_loss=0., ext_value_loss=0., int_value_loss=0.,
                   rnd_loss=0., entropy=0., approx_kl=0., clipfrac=0.,
                   grad_norm=0.)
        n_updates = 0
        ...
        for i in range(self.epoch):
            ...
            for j in range(int(len(s_batch) / self.batch_size)):
                ...
                loss.backward()
                # P2b: pass max_norm so the clipping actually fires (issue #17 fix)
                grad_norm = global_grad_norm_(
                    list(self.model.parameters()) + list(self.rnd.predictor.parameters()),
                    self.clip_grad_norm)
                self.optimizer.step()

                # --- accumulate metrics ---
                with torch.no_grad():
                    approx_kl = (log_prob_old[sample_idx] - log_prob).mean().item()
                    cf = ((ratio - 1.0).abs() > self.ppo_eps).float().mean().item()
                acc['policy_loss']    += actor_loss.item()
                acc['ext_value_loss'] += critic_ext_loss.item()
                acc['int_value_loss'] += critic_int_loss.item()
                acc['rnd_loss']       += forward_loss.item()
                acc['entropy']        += entropy.item()
                acc['approx_kl']      += approx_kl
                acc['clipfrac']       += cf
                acc['grad_norm']      += float(grad_norm)
                n_updates += 1

        # average over all mini-batch updates
        metrics = {k: v / max(n_updates, 1) for k, v in acc.items()}

        # explained variance for both value heads (Burda Appendix A)
        with torch.no_grad():
            _, ve, vi = self.model(s_batch)
            def _ev(pred, target):
                pred = pred.squeeze().detach().cpu().numpy()
                tgt  = target.detach().cpu().numpy()
                return float(1 - np.var(tgt - pred) / (np.var(tgt) + 1e-8))
            metrics['explained_var_ext'] = _ev(ve, target_ext_batch)
            metrics['explained_var_int'] = _ev(vi, target_int_batch)
